...
data {
  ...
  //added solitary covariates
  int<lower=1> K_muSolitary_SOcounts;  // number of population-level effects
  matrix[N_SOcounts, K_muSolitary_SOcounts] X_muSolitary_SOcounts;  // population-level design matrix
  int<lower=1> Ksp_muSolitary_SOcounts;  // number of special effects terms
  int<lower=1> Imo_muSolitary_SOcounts;  // number of monotonic variables
  int<lower=1> Jmo_muSolitary_SOcounts[Imo_muSolitary_SOcounts];  // length of simplexes
  int Xmo_muSolitary_SOcounts_1[N_SOcounts];  // monotonic variable
  vector[Jmo_muSolitary_SOcounts[1]] con_simo_muSolitary_SOcounts_1;  // prior concentration of monotonic simplex
  ...
  //added solitary effects

  // data for group-level effects of ID 1
  int<lower=1> N_21;  // number of grouping levels
  int<lower=1> M_21;  // number of coefficients per level
  int<lower=1> J_21_SOcounts[N_SOcounts];  // grouping indicator per observation
  // group-level predictor values
  vector[N_SOcounts] Z_21_muSolitary_SOcounts_1;
  // data for group-level effects of ID 2
  int<lower=1> N_22;  // number of grouping levels
  int<lower=1> M_22;  // number of coefficients per level
  int<lower=1> J_22_SOcounts[N_SOcounts];  // grouping indicator per observation
  // group-level predictor values
  vector[N_SOcounts] Z_22_muSolitary_SOcounts_1;
  // data for group-level effects of ID 3
  int<lower=1> N_23;  // number of grouping levels
  int<lower=1> M_23;  // number of coefficients per level
  int<lower=1> J_23_SOcounts[N_SOcounts];  // grouping indicator per observation
  matrix[N_23, N_23] Lcov_23;  // cholesky factor of known covariance matrix
  // group-level predictor values
  vector[N_SOcounts] Z_23_muSolitary_SOcounts_1;
  // data for group-level effects of ID 4
  int<lower=1> N_24;  // number of grouping levels
  int<lower=1> M_24;  // number of coefficients per level
  int<lower=1> J_24_SOcounts[N_SOcounts];  // grouping indicator per observation
  // group-level predictor values
  vector[N_SOcounts] Z_24_muSolitary_SOcounts_1;
}
transformed data {
  ...
  //added solitary params
  int Kc_muSolitary_SOcounts = K_muMF_SOcounts - 1;
  matrix[N_SOcounts, Kc_muSolitary_SOcounts] Xc_muSolitary_SOcounts;  // centered version of X_muMF_SOcounts without an intercept
  vector[Kc_muMF_SOcounts] means_X_muSolitary_SOcounts;  // column means of X_muMF_SOcounts before centering
...
//added solitary params  
   for (i in 2:K_muSolitary_SOcounts) {
    means_X_muSolitary_SOcounts[i - 1] = mean(X_muSolitary_SOcounts[, i]);
    Xc_muSolitary_SOcounts[, i - 1] = X_muSolitary_SOcounts[, i] - means_X_muSolitary_SOcounts[i - 1];
  } 
...
parameters {
 ...
 //added solitary params
  vector[Kc_muSolitary_SOcounts] b_muSolitary_SOcounts;  // population-level effects
  simplex[Jmo_muSolitary_SOcounts[1]] simo_muSolitary_SOcounts_1;  // monotonic simplex
  vector[Ksp_muSolitary_SOcounts] bsp_muSolitary_SOcounts;  // special effects coefficients
...
transformed parameters {
  ...
  //added solitary params
  vector[N_21] r_21_muSolitary_SOcounts_1;  // actual group-level effects
  vector[N_22] r_22_muSolitary_SOcounts_1;  // actual group-level effects
  vector[N_23] r_23_muSolitary_SOcounts_1;  // actual group-level effects
  vector[N_24] r_24_muSolitary_SOcounts_1;  // actual group-level effects
  
  //intercepts
  real Intercept_muMF_SOcounts = Intercepts_SO[1];
  real Intercept_muMFF_SOcounts = Intercepts_SO[2];
  real Intercept_muFMM_SOcounts = Intercepts_SO[3];
  real Intercept_muFFMM_SOcounts = Intercepts_SO[4];
  real Intercept_muSolitary_SOcounts = -sum(Intercepts_SO);
  ...
  //added solitary params
  r_21_muSolitary_SOcounts_1 = (sd_21[1] * (z_21[1]));
  r_22_muSolitary_SOcounts_1 = (sd_22[1] * (z_22[1]));
  r_23_muSolitary_SOcounts_1 = (sd_23[1] * (Lcov_23 * z_23[1]));
  r_24_muSolitary_SOcounts_1 = (sd_24[1] * (z_24[1]));
...
model {
  ...
  if (!prior_only) {
    ...
    //added params
    // initialize linear predictor term
    vector[N_SOcounts] muSolitary_SOcounts = Intercept_muSolitary_SOcounts + Xc_muSolitary_SOcounts * b_muSolitary_SOcounts;
   ...
   for (n in 1:N_SOcounts) {
    ...
    for (n in 1:N_SOcounts) {
      // add more terms to the linear predictor
      muSolitary_SOcounts[n] += (bsp_muSolitary_SOcounts[1]) * mo(simo_muSolitary_SOcounts_1, Xmo_muSolitary_SOcounts_1[n]) + r_21_muSolitary_SOcounts_1[J_21_SOcounts[n]] * Z_21_muSolitary_SOcounts_1[n] + r_22_muSolitary_SOcounts_1[J_22_SOcounts[n]] * Z_22_muSolitary_SOcounts_1[n] + r_23_muSolitary_SOcounts_1[J_23_SOcounts[n]] * Z_23_muSolitary_SOcounts_1[n] + r_24_muSolitary_SOcounts_1[J_24_SOcounts[n]] * Z_24_muSolitary_SOcounts_1[n];
    }
    ...
    for (n in 1:N_SOcounts) {
      mu_SOcounts[n] = transpose([muSolitary_SOcounts[n], muMF_SOcounts[n], muMFF_SOcounts[n], muFMM_SOcounts[n], muFFMM_SOcounts[n]]);
    }
    for (n in 1:N_SOcounts) {
  ...
  target += normal_lpdf(b_muMF_SOcounts | 0,1);
 //target += normal_lpdf(Intercept_muMF_SOcounts | 0,1);
  target += dirichlet_lpdf(simo_muMF_SOcounts_1 | con_simo_muMF_SOcounts_1);
  target += normal_lpdf(bsp_muMF_SOcounts | 0,1);
  target += normal_lpdf(b_muMFF_SOcounts | 0,1);
  //target += normal_lpdf(Intercept_muMFF_SOcounts | 0,1);
  target += dirichlet_lpdf(simo_muMFF_SOcounts_1 | con_simo_muMFF_SOcounts_1);
  target += normal_lpdf(bsp_muMFF_SOcounts | 0,1);
  target += normal_lpdf(b_muFMM_SOcounts | 0,1);
  //target += normal_lpdf(Intercept_muFMM_SOcounts | 0,1);
  target += dirichlet_lpdf(simo_muFMM_SOcounts_1 | con_simo_muFMM_SOcounts_1);
  target += normal_lpdf(bsp_muFMM_SOcounts | 0,1);
  target += normal_lpdf(b_muFFMM_SOcounts | 0,1);
  //target += normal_lpdf(Intercept_muFFMM_SOcounts | 0,1);
  target += dirichlet_lpdf(simo_muFFMM_SOcounts_1 | con_simo_muFFMM_SOcounts_1);
  target += normal_lpdf(bsp_muFFMM_SOcounts | 0,1);
  target += normal_lpdf(b_IVSOint | 0,1);
  target += normal_lpdf(Intercept_IVSOint | 0,1);
  target += dirichlet_lpdf(simo_IVSOint_1 | con_simo_IVSOint_1);
  
  //added
  target += normal_lpdf(Intercepts_SO| 0,1);
  target += normal_lpdf(b_muSolitary_SOcounts | 0,1);
  //target += normal_lpdf(Intercept_muSolitary_SOcounts | 0,1);
  target += dirichlet_lpdf(simo_muSolitary_SOcounts_1 | con_simo_muSolitary_SOcounts_1);
  target += normal_lpdf(bsp_muSolitary_SOcounts | 0,1);
  
  target += normal_lpdf(bsp_IVSOint | 0,1);
  ...
  target += exponential_lpdf(sd_17 | 2);
  target += std_normal_lpdf(z_17[1]);
  target += exponential_lpdf(sd_18 | 2);
  target += std_normal_lpdf(z_18[1]);
  target += exponential_lpdf(sd_19 | 2);
  target += std_normal_lpdf(z_19[1]);
  target += exponential_lpdf(sd_20 | 2);
  target += std_normal_lpdf(z_20[1]);
  
  //added
  target += exponential_lpdf(sd_21 | 2);
  target += std_normal_lpdf(z_21[1]);
  target += exponential_lpdf(sd_22 | 2);
  target += std_normal_lpdf(z_22[1]);
  target += exponential_lpdf(sd_23 | 2);
  target += std_normal_lpdf(z_23[1]);
  target += exponential_lpdf(sd_24 | 2);
  target += std_normal_lpdf(z_24[1]);
  
}
generated quantities {
  // actual population-level intercept
  real b_muSolitary_SOcounts_Intercept = Intercept_muSolitary_SOcounts - dot_product(means_X_muSolitary_SOcounts, b_muSolitary_SOcounts);
  // actual population-level intercept
...
//predictions
  ...
  //no fixed effects
  vector[N_SOcounts] muMF_SOcounts2 = rep_vector(Intercept_muMF_SOcounts,N_SOcounts);
  // initialize linear predictor term
  vector[N_SOcounts] muMFF_SOcounts2 = rep_vector(Intercept_muMFF_SOcounts,N_SOcounts);
    // initialize linear predictor term
  vector[N_SOcounts] muFMM_SOcounts2 = rep_vector(Intercept_muFMM_SOcounts,N_SOcounts);
    // initialize linear predictor term
  vector[N_SOcounts] muFFMM_SOcounts2 = rep_vector(Intercept_muFFMM_SOcounts,N_SOcounts);
    // initialize linear predictor term
  vector[N_IVSOint] mu_IVSOint2 = rep_vector(Intercept_IVSOint,N_IVSOint);
    // initialize linear predictor term
  vector[N_SOcounts] muSolitary_SOcounts2 = rep_vector(Intercept_muSolitary_SOcounts,N_SOcounts);;
   ...
   for (n in 1:N_SOcounts) {
    ...
    for (n in 1:N_SOcounts) {
      // add more terms to the linear predictor
      muMF_SOcounts2[n] += r_1_muMF_SOcounts_1[J_1_SOcounts[n]] * Z_1_muMF_SOcounts_1[n] + r_2_muMF_SOcounts_1[J_2_SOcounts[n]] * Z_2_muMF_SOcounts_1[n] + r_3_muMF_SOcounts_1[J_3_SOcounts[n]] * Z_3_muMF_SOcounts_1[n] + r_4_muMF_SOcounts_1[J_4_SOcounts[n]] * Z_4_muMF_SOcounts_1[n];
    }
    for (n in 1:N_SOcounts) {
      // add more terms to the linear predictor
      muMFF_SOcounts2[n] += r_5_muMFF_SOcounts_1[J_5_SOcounts[n]] * Z_5_muMFF_SOcounts_1[n] + r_6_muMFF_SOcounts_1[J_6_SOcounts[n]] * Z_6_muMFF_SOcounts_1[n] + r_7_muMFF_SOcounts_1[J_7_SOcounts[n]] * Z_7_muMFF_SOcounts_1[n] + r_8_muMFF_SOcounts_1[J_8_SOcounts[n]] * Z_8_muMFF_SOcounts_1[n];
    }
    for (n in 1:N_SOcounts) {
      // add more terms to the linear predictor
      muFMM_SOcounts2[n] += r_9_muFMM_SOcounts_1[J_9_SOcounts[n]] * Z_9_muFMM_SOcounts_1[n] + r_10_muFMM_SOcounts_1[J_10_SOcounts[n]] * Z_10_muFMM_SOcounts_1[n] + r_11_muFMM_SOcounts_1[J_11_SOcounts[n]] * Z_11_muFMM_SOcounts_1[n] + r_12_muFMM_SOcounts_1[J_12_SOcounts[n]] * Z_12_muFMM_SOcounts_1[n];
    }
    for (n in 1:N_SOcounts) {
      // add more terms to the linear predictor
      muFFMM_SOcounts2[n] += r_13_muFFMM_SOcounts_1[J_13_SOcounts[n]] * Z_13_muFFMM_SOcounts_1[n] + r_14_muFFMM_SOcounts_1[J_14_SOcounts[n]] * Z_14_muFFMM_SOcounts_1[n] + r_15_muFFMM_SOcounts_1[J_15_SOcounts[n]] * Z_15_muFFMM_SOcounts_1[n] + r_16_muFFMM_SOcounts_1[J_16_SOcounts[n]] * Z_16_muFFMM_SOcounts_1[n];
    }
    //added solitary
    for (n in 1:N_SOcounts) {
      // add more terms to the linear predictor
      muSolitary_SOcounts2[n] += r_21_muSolitary_SOcounts_1[J_21_SOcounts[n]] * Z_21_muSolitary_SOcounts_1[n] + r_22_muSolitary_SOcounts_1[J_22_SOcounts[n]] * Z_22_muSolitary_SOcounts_1[n] + r_23_muSolitary_SOcounts_1[J_23_SOcounts[n]] * Z_23_muSolitary_SOcounts_1[n] + r_24_muSolitary_SOcounts_1[J_24_SOcounts[n]] * Z_24_muSolitary_SOcounts_1[n];
    }
    for (n in 1:N_IVSOint) {
      // add more terms to the linear predictor
      mu_IVSOint2[n] += r_17_IVSOint_1[J_17_IVSOint[n]] * Z_17_IVSOint_1[n] + r_18_IVSOint_1[J_18_IVSOint[n]] * Z_18_IVSOint_1[n] + r_19_IVSOint_1[J_19_IVSOint[n]] * Z_19_IVSOint_1[n] + r_20_IVSOint_1[J_20_IVSOint[n]] * Z_20_IVSOint_1[n];
    }

...
tree_file <- "AN54_Di_Fiore_Supplemental_Kuderna_et_al_2023_Phylo.tree"
...
d <- read_csv("AN54_Di_Fiore_Supplemental_Kuderna_et_al_2023_Taxa.csv", col_names = TRUE) ## adds in other taxonomic levels for taxa in Kuderna et al (2023) data set
d <- d |>
  rowwise() |>
...
saveRDS(p, "output/Supplemental Figure 2.rds")
ggsave("output/Supplemental Figure 2.pdf", width = 12, height = 9, units = "in", dpi = 400)
...
common_data <- read_csv("AN54_Di_Fiore_Supplemental_Olivier_et_al_2024_Data.csv", col_names = TRUE)
...
comparison_data <- read_excel("AN54_Di_Fiore_Supplemental_Table2.xlsx", sheet = 1, col_names = TRUE) |>
  rowwise() |>
...
## Kappeler & Pozzi (2019) 3 state - S<P<G
## recode P, G -> P
d[[3]] <- combined_data |>
...
n[[3]] <- "Kappeler & Pozzi (2019) 3 state (S<P<G)"

## Kappeler & Pozzi (2019) 3 state - S<P<G + outgroup
## adding in additional data for outgroup taxa
## recode P, G -> P
d[[4]] <- combined_data |>
  ...
  mutate(`Genus` = str_split(`Species`, "_")[[1]][1]) |>
  mutate(`character_data` = if_else(`character_data` == "P, G", "P", `character_data`))
n[[4]] <- "Kappeler & Pozzi (2019) 3 state (S<P<G) + outgroup"
...
## Shultz et al (2011) 3 state - S<P<G
...
n[[7]] <- "Shultz et al (2011) 3 state (S<P<G)"

## Shultz et al (2011) 3 state - S<P<G + outgroup
## adding in additional data for outgroup taxa
## recode S, P -> S; S, G -> S; S, P, G -> S
d[[8]] <- combined_data |>
  ...
  mutate(`character_data` = if_else(`character_data` == "S, G", "S", `character_data`)) |>
  mutate(`character_data` = if_else(`character_data` == "S, P, G", "S", `character_data`))
n[[8]] <- "Shultz et al (2011) 3 state (S<P<G) + outgroup"

## Müller & Thalmann (2000) 3 state - S<P<G
## recode P, G -> P
d[[9]] <- combined_data |>
  ...
  mutate(`Genus` = str_split(`Species`, "_")[[1]][1]) |>
  mutate(`character_data` = if_else(`character_data` == "P, G", "P", `character_data`))
n[[9]] <- "Müller & Thalmann (2000) 3 state (S<P<G)"

## Müller & Thalmann (2000) 3 state - S<P<G + outgroup
## adding in additional data for outgroup taxa
## recode P, G -> P
d[[10]] <- combined_data |>
  ...
  mutate(`Genus` = str_split(`Species`, "_")[[1]][1]) |>
  mutate(`character_data` = if_else(`character_data` == "P, G", "P", `character_data`))
n[[10]] <- "Müller & Thalmann (2000) 3 state (S<P<G) + outgroup"

## Müller & Thalmann (2000) 5 state - S<P<G
## recode D-P, D-G -> D-P
d[[11]] <- combined_data |>
...
n[[11]] <- "Müller & Thalmann (2000) 5 state (S<P<G)"

## Müller & Thalmann (2000) 5 state - S<P<G + outgroup
## adding in additional data for outgroup taxa
## recode D-P, D-G -> D-P
d[[12]] <- combined_data |>
  ...
  mutate(`character_data` = if_else(`character_data` == "D-P, D-G", "D-P", `character_data`)) |>
  mutate(`character_data` = if_else(`Species` == "Pongo_pygmaeus", "D-G", `character_data`))
n[[12]] <- "Müller & Thalmann (2000) 5 state (S<P<G) + outgroup"

## Kappeler & Pozzi (2019) 3 state - G>P>S
## recode P, G -> G
d[[13]] <- combined_data |>
  filter(!is.na(`Kappeler & Pozzi (2019) 3 state`)) |>
  mutate(character_data = `Kappeler & Pozzi (2019) 3 state`) |>
...
n[[13]] <- "Kappeler & Pozzi (2019) 3 state (G>P>S)"

## Kappeler & Pozzi (2019) 3 state - G>P>S + outgroup
## adding in additional data for outgroup taxa
## recode P, G -> P
d[[14]] <- combined_data |>
  filter(!is.na(`Kappeler & Pozzi (2019) 3 state`) | !is.na(`Outgroup 3 state`)) |>
  ...
  mutate(`Genus` = str_split(`Species`, "_")[[1]][1]) |>
  mutate(`character_data` = if_else(`character_data` == "P, G", "P", `character_data`))
n[[14]] <- "Kappeler & Pozzi (2019) 3 state (G>P>S) + outgroup"

## Shultz et al (2011) 3 state - G>P>S
...
n[[15]] <- "Shultz et al (2011) 3 state (G>P>S)"

## Shultz et al (2011) 3 state - G>P>S + outgroup
## adding in additional data for outgroup taxa
# recode S, P -> P; S, G -> G; S, P, G -> G
d[[16]] <- combined_data |>
  filter(!is.na(`Shultz et al (2011) 3 state`) | !is.na(`Outgroup 3 state`)) |>
  ...
  mutate(`character_data` = if_else(`character_data` == "S, G", "G", `character_data`)) |>
  mutate(`character_data` = if_else(`character_data` == "S, P, G", "G", `character_data`))
n[[16]] <- "Shultz et al (2011) 3 state (G>P>S) + outgroup"

## Müller & Thalmann (2000) 3 state - G>P>S
## recode P, G -> G
d[[17]] <- combined_data |>
  filter(!is.na(`Müller & Thalmann (2000) 3 state`)) |>
  mutate(character_data = `Müller & Thalmann (2000) 3 state`) |>
  ...
  mutate(`Genus` = str_split(`Species`, "_")[[1]][1]) |>
  mutate(`character_data` = if_else(`character_data` == "P, G", "G", `character_data`))
n[[17]] <- "Müller & Thalmann (2000) 3 state (G>P>S)"

## Müller & Thalmann (2000) 3 state - G>P>S + outgroup
## adding in additional data for outgroup taxa
## recode P, G -> G
d[[18]] <- combined_data |>
  filter(!is.na(`Müller & Thalmann (2000) 3 state`) | !is.na(`Outgroup 3 state`)) |>
  ...
  mutate(`Genus` = str_split(`Species`, "_")[[1]][1]) |>
  mutate(`character_data` = if_else(`character_data` == "P, G", "G", `character_data`))
n[[18]] <- "Müller & Thalmann (2000) 3 state (G>P>S) + outgroup"

## Müller & Thalmann (2000) 5 state - G>P>S
## recode D-P, D-G -> D-G
d[[19]] <- combined_data |>
  filter(!is.na(`Müller & Thalmann (2000) 5 state`)) |>
  mutate(character_data = `Müller & Thalmann (2000) 5 state`) |>
  ...
  mutate(`character_data` = if_else(`Species` == "Pongo_pygmaeus", "D-G", `character_data`))
n[[19]] <- "Müller & Thalmann (2000) 5 state (G>P>S)"

## Müller & Thalmann (2000) 5 state - G>P>S + outgroup
## adding in additional data for outgroup taxa
## recode D-P, D-G -> D-G
d[[20]] <- combined_data |>
  filter(!is.na(`Müller & Thalmann (2000) 5 state`) | !is.na(`Outgroup 5 state`)) |>
  ...
  mutate(`character_data` = if_else(`character_data` == "D-P, D-G", "D-G", `character_data`)) |>
  mutate(`character_data` = if_else(`Species` == "Pongo_pygmaeus", "D-G", `character_data`))
n[[20]] <- "Müller & Thalmann (2000) 5 state (G>P>S) + outgroup"
...
tree_file <- "AN54_Di_Fiore_Supplemental_Olivier_et_al_2024_Phylo.nex"
...
Olivier_et_al_2024_phylogeny <- tree ## hold this tree!
...
for (i in seq(1, length(d), 2)){
  ...
  tree <- Olivier_et_al_2024_phylogeny
...
is.binary(tree)
tree$node.label <- NULL
Kuderna_et_al_2023_phylogeny <- tree ## hold this tree!

## store tree
write.tree(Kuderna_et_al_2023_phylogeny, "output/Kuderna_et_al_2023_Phylogeny.tree") ## store for later
...
for (i in seq(1, length(d))){
  ...
  ## create treedata.table to merge tree and data and drop tips that are missing in dataset and data that are missing in tree
  tree <- Kuderna_et_al_2023_phylogeny
  t <- as.treedata.table(tree = tree, data = as.data.frame(d[[i]]), name_column = "Species")
  phy <- t$phy ## get the phylogeny
  is.rooted.phylo(phy) ## check if it's rooted
  is.binary(phy) ## check if it's binary
...
state_names <- c("G = group-living", "P = pair-living", "S = solitary")

# write this figure as a PDF file 11in x 9in... first create a PDF graphic device
pdf("output/Supplemental Figure 3.pdf", width = 11, height = 9, bg = "white")
...
## Supplemental Figure 4a (S<P<G results)
